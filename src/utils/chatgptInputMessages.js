...
export function createPromptQueryMessages(
  initialQuery,
  promptMessage,
  contextSentences = undefined
) {
  var systemMessage =
    "You will receive 2 messages. " +
    "The 1st one will contain a Bigquery SQL query (or will be empty). " +
  ...
  systemMessage +=
    "Let's think step by step on how to achieve final sql. Always answer in following format. /*% Steps => ${Write the steps you followed to achieve final query here}  %*/  ${Write formatted final sql query here}. Answer you don't know when you are not sure in the same format.";
  return [
    { role: "system", content: systemMessage },
    { role: "user", content: initialQuery },
    {
      ...
      content:
        promptMessage +
        "\n\n Remember, I only want a raw SQL query in sql_query object of final response  as a response: no comment, no explanation, no I'm sorry or I don't have enough info. Just write the most likely to work Bigquery SQL query. Remember it should work in Bigquery",
    },
  ];
}

export function createDatabaseErrorMessages(
  initialQuery,
  errorMessage,
  contextSentences = undefined
) {
